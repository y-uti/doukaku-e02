...
function main($argc, $argv)
{
    ...
    foreach ($problems as $problem) {
        ...
        list ($min, $max) = solve_problem($n, $cells);
        echo $max != -1 ? "$min,$max\n" : "-\n";
    }
}
...
function read_problem($problem)
{
    ...
    $cells = array_map(function ($encoded) use ($CHARS) {
        $x = strpos($CHARS, $encoded[0]);
        $y = strpos($CHARS, $encoded[1]);
        return array($x, $y);
    }, explode(',', $data));
...
function solve_problem($n, $cells, $size = 62)
{
